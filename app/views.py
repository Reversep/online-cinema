...
from .forms import CategoryForm, MovieForm, MovieUpdateForm, CategoryUpdateForm, UserLoginForm, UserRegisterForm
...
def admin_movie_update(movie_id):
    movie = Movie.query.get(movie_id)
    form = MovieUpdateForm(meta={'csrf': False}, obj=movie)
    if request.method == 'POST':
...
def admin_category_update(category_id):
    ...
    form = CategoryUpdateForm(meta={'csrf': False}, obj=category)
    if request.method == 'POST':
